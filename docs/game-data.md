...
| sources | 採集節點／時段、無人島採集座標與種植／畜牧來源、金幣商店／NPC、怪物掉落／座標／名稱，以及所引用地圖／四語地名 | 第二階段；包含怪物，不等進備料台才載入 |
...
- 建築材料有無人島採集座標者歸採集；屯貨倉庫素材 37578、37579、37580、37581、37582、39894 歸其他，不杜撰配方或狩獵來源。
- 開拓特殊來源由 `src/services/islandSources.ts` 統一解讀：作物依同一 Teamcraft SHA 的 `island-crops.json` 產物 ID、牧場產物依 `island-animals.json` 全部 `rewards`（包含兩種產物）投影為 sources 的 `islandProduction`。不是按名稱、連續 ID 或種子推測；屯貨倉庫維持上述既有名單。三類都歸「其他」，備料台、待辦及 HTML 匯出顯示各自標記，不改成普通採集或狩獵。
- `islandProduction` 是 formatVersion 2 的可選擴充；舊快取缺此欄位仍可使用，但須套用新版資料才顯示種植／畜牧標記。Generator 必須下載新增的兩份來源；既有本地快照缺檔時，用相同 `--ref <SHA>` 補齊再產包。
