...
套裝名稱只列等級、品級、職業與件數，不加「含武器／主副手」括號註記。無論是否輸入搜尋條件，皆以職業套裝及包含工具的全職業套裝優先，原有共用裝備筆記排在後面；各組內先依 Lv 由低到高，再依套裝最高 iLv 由低到高排序，相同條件維持原順序。混裝品級取名稱列出的最大值，例如 i190+195 取 195。排序鍵只解析一次，優先順序與完整列表在建立索引時準備好，不逐次搜尋排序或讀取物品清單。
...
- 生活練等：Lv.51 i65、Lv.61 i180 僅提供巧匠與大地各五件防具，生成器的 `standaloneFiles` 明確排除這兩份檔案，不補舊工具或飾品。Lv.51 防具可供全職業穿戴，巧匠／大地用途採核對過的物品清單，不能僅按 `equipJobs` 自動分類。
- Lv.71 i330、Lv.81 i480、Lv.91 i610 提供同級共用十件、十一個職業套裝及巧匠二十六／大地十五件套，各階段十五筆。戒指為同物品兩枚，漁師不補副手。
- 資料片入門戰鬥製作裝沿用既有 Lv.50 i115、Lv.60 i255、Lv.70 i385、Lv.80 i515、Lv.90 i645 筆記，不重複新增。標題標示穿戴等級，不能把代表武器的 X1 配方等級當成穿戴等級。
...
共用／獨立筆記 123 筆（原有 113 筆及生活練等基底 10 筆），加上 380 筆職業／全職業組合，共 503 筆。生活練等新增 49 筆，其中 39 筆使用組合。新增組合資料只記錄差異，名稱依模板生成；搜尋和計數不展開物品。卡片第一次渲染、收藏或開啟時才產生該筆物品陣列，且各筆陣列彼此獨立。維持每頁二十筆，不一次渲染全部卡片。搜尋改字立即回第一頁，避免上一頁位置造成空白結果。

最初 454 筆版本的量測基準：組合 JSON 為 36,843 bytes，gzip 6,547 bytes；341 筆完整展開 JSON 為 195,507 bytes，gzip 16,672 bytes。這是新增資料模型的比較，並非整站下載量。當時 Windows Node.js 本機量測：454 筆名稱／索引建立約 1.7 ms，10,000 次搜尋的 p95 約 0.013 ms，搜尋觸發物品 getter 次數為零；不能視為所有裝置或後續版本的速度保證。
